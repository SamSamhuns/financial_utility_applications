...
class trade_prob_tree:
    def __init__(self, value_high, value_low):
        ...
        # Prob that the stock value is high at the end of the day
        self.value_high_prob = value_high
        # Prob that the stock value is low at the end of the day
        self.value_low_prob = value_low

        # Prob that the trader is informed given the stock val is low
        self.informed_given_low = 0.1
        # Prob that the trader is uninformed given the stock val is low
        self.uninformed_given_low = 0.4
        # Prob that the trader is informed given the stock val is high
        self.informed_given_high = 0.1
        # Prob that the trader is uinformed given the stock val is high
        self.uninformed_given_high = 0.4

        # Prob of a buy given the trader is informed given the stock val is low
        self.buy_given_informed_low = 0
        # Prob of a sell given the trader is informed given the stock val is low
        self.sell_given_informed_low = 0.1
        # Prob of a buy given the trader is uninformed given the stock val is low
        self.buy_given_uninformed_low = 0.2
        # Prob of a sell given the trader is uninformed given the stock val is low
        self.sell_given_uninformed_low = 0.2
        # Prob of a buy given the trader is informed given the stock val is high
        self.buy_given_informed_high = 0.1
        # Prob of a sell given the trader is informed given the stock val is high
        self.sell_given_informed_high = 0
        # Prob of a buy given the trader is uninformed given the stock val is high
        self.buy_given_uninformed_high = 0.2
        # Prob of a sell given the trader is uninformed given the stock val is high
        self.sell_given_uninformed_high = 0.2
...
if len(sys.argv) == 1:
    trade_seq_str = (
        input('Enter the trade sequence (Format bssbssbbb) :')).lower().replace(' ', '')
...
# Creating a trade_prob_tree object
main_tree = trade_prob_tree(0.5, 0.5)
...
# Proportion of uninformed traders
uninformed_prop = 1 - informed_prop
...
trade_type = 'Open'
# [ trade_type, value_high, value_low ]
trade_seq_array = [
    [trade_type, main_tree.value_high_prob, main_tree.value_low_prob]]
...
for trade in trade_seq_str:
    ...
    main_tree.buy_given_informed_low = prob_informed_buy_if_low * \
        main_tree.informed_given_low
    main_tree.sell_given_informed_low = prob_informed_sell_if_low * \
        main_tree.informed_given_low
    main_tree.buy_given_uninformed_low = prob_uninformed_buy_sell * \
        main_tree.uninformed_given_low
    main_tree.sell_given_uninformed_low = prob_uninformed_buy_sell * \
        main_tree.uninformed_given_low
    main_tree.buy_given_informed_high = prob_informed_buy_if_high * \
        main_tree.informed_given_high
    main_tree.sell_given_informed_high = prob_informed_sell_if_high * \
        main_tree.informed_given_high
    main_tree.buy_given_uninformed_high = prob_uninformed_buy_sell * \
        main_tree.uninformed_given_high
    main_tree.sell_given_uninformed_high = prob_uninformed_buy_sell * \
        main_tree.uninformed_given_high

    # Add the probability to the trade_seq_array
    trade_seq_array.append([trade_type, round(
        main_tree.value_high_prob, 5), round(main_tree.value_low_prob, 5)])
...
print(('%s \t %s \t %s') % ('Trade type', 'High Value Prob',
                            'Low Value Prob'), file=buy_sell_prob_output)
# print all the probablities into an output file
for trade_arr in trade_seq_array:
    print(trade_arr[0], trade_arr[1], trade_arr[2],
          file=buy_sell_prob_output, sep='\t\t')

buy_sell_prob_output.close()
# generate a numpy array for holding the high/low probability information
trade_arr_numpy = np.array(
    [[i, trade_seq_array[i][1], trade_seq_array[i][2]] for i in range(len(trade_seq_array))])
...
high_bar = ax.bar(
    ind - width / 2, trade_arr_numpy[:, 1], width, color='green', label='High Value probability')
low_bar = ax.bar(
    ind + width / 2, trade_arr_numpy[:, 2], width, color='#bb0000', label='Low Value probability')
# getting the x ticks of buys and sells
x_label_ticks = np.array([trade_seq_array[i][0]
                          for i in range(len(trade_seq_array))])
